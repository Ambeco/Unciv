...
object TargetHelper {
    ...
    fun getAttackableEnemies(
        ...
        val tilesToAttackFrom = 
            if (stayOnTile || unit.baseUnit.movesLikeAirUnits)
                PathsToTilesWithinTurn.of(unit.currentTile, unit.currentTile, unit.currentMovement)
            else unitDistanceToTiles

        val tilesWithEnemies: HashSet<Tile> = HashSet()
        val tilesWithoutEnemies: HashSet<Tile> = HashSet()
        tilesToAttackFrom.forEachTile { reachableTile ->
            // tiles we'll still have energy after we reach there
            val movementLeft = getMovementAfterUnitMoves(reachableTile, tilesToAttackFrom.getMovement(reachableTile), unitMustBeSetUp, unit)
            ...
            if (unit.baseUnit.isMelee() && unit.isEscorting()) {
                ...
                if (!escortingUnit.movement.canReachInCurrentTurn(reachableTile)
                    || escortingUnit.currentMovement - escortingUnit.movement.getDistanceToTiles().getMovement(reachableTile) <= 0f) 
                    return@forEachTile
            }
    ...
    @Readonly
    private fun getMovementAfterUnitMoves(tile: Tile, totalMovement: Float, unitMustBeSetUp: Boolean, unit: MapUnit): Float {
        ...
        val movementLeft =
            unit.currentMovement - totalMovement - movementPointsToExpendBeforeAttack
