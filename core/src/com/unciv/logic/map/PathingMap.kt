...
class PathingMap(
     ...
     */
    @Cache
    private val tilesReached = Array<ParentTileAndTotalMovement?>(startingPoint.tileMap.tileList.size) { null }
    
    @Cache
    private lateinit var tilesSameTurn: LinkedHashMap<Tile, ParentTileAndTotalMovement>
     ...
     */
    fun getMovementToTilesAtPosition(): LinkedHashMap<Tile, ParentTileAndTotalMovement>  {
        if (startingPoint != lazyStartingPoint() || currentMovement != lazyCurrentMovement()) {
            clear()
        }
        ...
        val newTilesSameTurn = LinkedHashMap<Tile, ParentTileAndTotalMovement>()
...
internal class AStarPathfinder(
    ...
    private fun asPrioritizedNode(node: ParentTileAndTotalMovement): PrioritizedNode {
        val movementSoFar = (node.turns + 1) * fullMovement - node.totalMovement
    ...
    private fun createNextNode(currentNode: ParentTileAndTotalMovement, neighborTile: Tile): ParentTileAndTotalMovement {
        ...
        if (cost <= currentNode.totalMovement)
            return ParentTileAndTotalMovement(
                neighborTile, currentNode.tile, currentNode.turns,
                (currentNode.totalMovement - cost).coerceAtLeast(0f)
            )
        ...
        return ParentTileAndTotalMovement(
            ...
            (fullMovement - cost).coerceAtLeast(0f)
        )
    }
