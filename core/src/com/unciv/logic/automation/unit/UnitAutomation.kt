...
object UnitAutomation {
    ...
    internal fun tryExplore(unit: MapUnit): Boolean = timeThis("tryExplore") {
        ...
        val explorableTilesThisTurn =
                unit.movement.getDistanceToTiles().filter {tile-> isGoodTileToExplore(unit, tile, unitVisibilityRange) }
        if (explorableTilesThisTurn.any()) {
            val bestTile = explorableTilesThisTurn
                .maxTileBy {tile -> tile.tileHeight + tile.getTilesAtDistance(unit.getVisibilityRange()).count { !it.isExplored(unit.civ) }.toFloat()}
    ...
    private fun tryFogBust(unit: MapUnit): Boolean {
        ...
        val reachableTilesThisTurn =
                unit.movement.getDistanceToTiles().filter {tile -> isGoodTileForFogBusting(unit, tile) }
    ...
    private fun tryRetreat(unit: MapUnit): Boolean {
        ...
        val sortedTilesToRetreatTo: Sequence<Tile> = if (closestCity != null) {
            ...
            unitDistanceToTiles.tilesSortedBy {tile -> tile.aerialDistanceTo(closestCity.getCenterTile()).toFloat() }
        } else {
            ...
            unitDistanceToTiles.tilesSortedByDescending { tile -> unit.civ.threatManager.getDistanceToClosestEnemyUnit(tile, 3, false).toFloat() }
        }
    ...
    private fun tryHealUnit(unit: MapUnit): Boolean {
        ...
        val viableTilesForHealing = unitDistanceToTiles
                .filter {tile -> tile !in dangerousTiles && unit.movement.canMoveTo(tile) }
        val hasTilesForHealing = viableTilesForHealing.anyTile {tile -> unit.rankTileForHealing(tile) > 0 }
        ...
        val bestTileForHealing = viableTilesForHealing.maxTileBy { tile ->
    ...
    fun tryPillageImprovement(unit: MapUnit, onlyPillageToHeal: Boolean = false): Boolean {
        ...
        val tilesThatCanWalkToAndThenPillage = unitDistanceToTiles
            .filter {tile -> unitDistanceToTiles.getMovement(tile) < unit.currentMovement
        ...
        val tileToPillage = tilesThatCanWalkToAndThenPillage.maxTileByOrNull { tile -> tile.getDefensiveBonus(false, unit) }!!
